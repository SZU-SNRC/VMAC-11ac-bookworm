 ...
 *  else if type is data 发data响应别人的interest
 *      look up tx table for the same encoding
 *      if entry does not exist
...
void vmac_tx(struct sk_buff *skb, u64 enc, u8 type, u16 seqtmp, u8 rate, u8 bw, u8 sgi, u8 stream, _adapter *mon_adapter)
{
...
#endif
    #ifdef DEBUG_VMAC
        // printk(KERN_INFO "VMAC: type of frame %d, %s %s %d\n", type, __FILE__, __FUNCTION__, __LINE__);
    #endif
    if (type == VMAC_HDR_INTEREST)
    {
        vmacr = find_rx(RX_TABLE, enc);
        #ifdef DEBUG_VMAC
                printk(KERN_INFO "TRANSMIT AN INTEREST, %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
        #endif
        if (!vmacr || vmacr == NULL)
        {
#ifdef DEBUG_VMAC
            printk(KERN_INFO "VMACTX: making new entry %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
#endif
            ...
            // #ifdef DEBUG_VMAC
            //             printk(KERN_INFO "VMAC: about to call rx %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
            // #endif
            add_rx(vmacr);
            ...
            // #ifdef DEBUG_VMAC
            //             printk(KERN_INFO "CALLING MOD TIMER %u, %s %s %d\n", vmacr->key, __FILE__, __FUNCTION__, __LINE__);
            // #endif
        ...
        // #ifdef DEBUG_VMAC
        //         printk(KERN_INFO "VMAC: about to mod timer %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
        // #endif
        mod_timer(&vmacr->enc_timeout, jiffies + msecs_to_jiffies(10000)); /* FIXME: Needs to be defaulted from vmac.h or userspace */

        memcpy(skb_push(skb, sizeof(struct vmac_hdr)), &vmachdr, sizeof(struct vmac_hdr));
        #ifdef DEBUG_VMAC
                printk(KERN_INFO "VMAC Interest Logic Completed %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
        #endif
    } // Data
    else if (type == VMAC_HDR_DATA)
    {
        #ifdef DEBUG_VMAC
                printk(KERN_INFO "TRANSMIT AN DATA, %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
        #endif
        ...
        mod_timer(&vmact->enc_timeout, jiffies + msecs_to_jiffies(10000)); /* FIXME: Needs to be defaulted from vmac.h or userspace */
        ...
        // {
        //     #ifdef DEBUG_VMAC
        //             printk(KERN_INFO "FREEING?! %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
        //     #endif
        //     tmp2 = (vmact->retransmission_buffer[ddr.seq % WINDOW_TX]);
        // }
        #ifdef DEBUG_VMAC
                printk(KERN_INFO "Making skb_copy %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
        #endif
        // if (tmp1)
        // {
    ...
    {
        // #ifdef DEBUG_VMAC
        //         printk(KERN_INFO "VMACTX: TEST3 %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
        // #endif
        hdr.duration_id = 0;
        memcpy(skb_push(skb, sizeof(struct vmac_hdr)), &vmachdr, sizeof(struct vmac_hdr));
    }
    ...
    #ifdef DEBUG_VMAC
        printk(KERN_INFO "VMAC_MID: Data rate: %02x, %s %s %d\n", rate, __FILE__, __FUNCTION__, __LINE__);
        printk(KERN_INFO "seq: %d, bw: %d, sgi: %d, stream: %d\n", seq, bw, sgi, stream);
    #endif
    vmac_low_tx(skb, seq, rate, bw, sgi, stream, mon_adapter);
    printk(KERN_INFO "TX DONE....... %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
}
...
void vmac_low_tx(struct sk_buff *skb, u16 seq, u8 rate, u8 bw, u8 sgi, u8 stream, _adapter *mon_adapter)
{
    ...
    // printk(KERN_INFO "WE ARE HERE AT FAKE SEND using source mac ADDRESS~ %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
    ...
    printk(KERN_INFO "xmit_mo VALUe we got is %d, %s %s %d\n", ret, __FILE__, __FUNCTION__, __LINE__);
}
...
s32 xmit_mo(struct sk_buff *skb, _adapter *padapter, u8 rate, u8 bw, u8 sgi, u8 stream)
{
    ...
    u8 category, action;
    int type = -1;
    // printk(KERN_INFO "Sending at rate %u, %u, %u, %s %s %d\n", rate, bw, sgi, __FILE__, __FUNCTION__, __LINE__);
    if (skb)
